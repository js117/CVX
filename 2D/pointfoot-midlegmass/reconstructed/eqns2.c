...
/* ************************ */
    if (flag == 's') {
        ...
        /* Compute dynamics: */
        tmp[1] = 2./L;
        tmp[2] = 1/(-3.*M - 4.*Mp + 2.*M*cos(2.*(q[0] - q[1])));
        tmp[3] = -2.*g*(M + Mp)*sin(beta + q[0]);
        tmp[4] = L*M*(2.*cos(q[0] - q[1])*pow(q[2],2.) - pow(q[3],2.))*sin(q[0] - q[1]);
        tmp[5] = g*M*sin(beta - q[0] + 2.*q[1]);
        out[2] = tmp[1]*tmp[2]*(tmp[3] + tmp[4] + tmp[5]);
        tmp[1] = 1/L;
        tmp[2] = 1/(-3.*M - 4.*Mp + 2.*M*cos(2.*(q[0] - q[1])));
        tmp[3] = 2.*L*(5.*M + 4.*Mp)*pow(q[2],2.)*sin(q[0] - q[1]) - 2.*L*M*pow(q[3],2.)*sin(2.*(q[0] - q[1]));
        tmp[4] = -2.*g*(3.*M + 2.*Mp)*sin(beta + 2.*q[0] - q[1]) + 4.*g*(M + Mp)*sin(beta + q[1]);
        out[3] = tmp[1]*tmp[2]*(tmp[3] + tmp[4]);
        tmp[1] = -8.*c;
        tmp[2] = (6.*M + 4.*Mp)*(L*L) + ((5.*M + 4.*Mp)*cos(2.*q[0]) - 8.*M*cos(q[0])*cos(q[1]) + M*cos(2.*q[1]))*(L*L);
        tmp[3] = q[4];
        out[4] = (tmp[1]*tmp[3])/tmp[2];
        /* End dynamics. */
