...
/* EDIT FOR EACH MODEL */
    const int dim = 4;
    char *paramNames[5] = {"M", "Mp", "g", "L", "slope"};
    mxArray *params;
    double M, Mp, g, L, slope; /* Model parameters */    
...
/* ************ */
    ...
    for (i=0 ; i<5 ; i++) {
        ...
        switch(i) {
            ...
            case 4: slope = *mxGetPr(params); break;
...
/* ************************ */
    if (flag == 's') {
        ...
        /* EDIT FOR EACH MODEL */        
        /* Compute dynamics: */
        out[2] = (-2.*(2.*g*(M + Mp)*sin(q[0]) + g*M*sin(q[0] - 2.*q[1]) + L*M*pow(q[3],2.)*sin(q[0] - q[1]) - L*M*pow(q[2],2.)*sin(2.*(q[0] - q[1]))))/(L*(-3.*M - 4.*Mp + 2.*M*cos(2.*(q[0] - q[1]))));
        out[3] = (2.*L*(5.*M + 4.*Mp)*pow(q[2],2.)*sin(q[0] - q[1]) - 2.*L*M*pow(q[3],2.)*sin(2.*(q[0] - q[1])) - 2.*g*(3.*M + 2.*Mp)*sin(2.*q[0] - q[1]) + 4.*g*(M + Mp)*sin(q[1]))/(L*(-3.*M - 4.*Mp + 2.*M*cos(2.*(q[0] - q[1]))));
        /* End dynamics. */
        ...
        out[3] = ((M - 4.*(M + Mp)*cos(2.*(q[0] - q[1])))*q[2] + 2.*M*cos(q[0] - q[1])*q[3])/(-3.*M - 4.*Mp + 2.*M*cos(2.*(q[0] - q[1])));
        /* End impact. */
