...
void putInvalid() {
    ...
    double *val;
    *val = 1;
    mxSetPr(invalidValue, val);
    mexPrintf("mxArray invalidValue = %f", *mxGetPr(invalidValue));
    
    if (mexPutVariable("global", "invalidResults", invalidValue) == 1) {
        dieHorribly(-1);
    }
}
...
/* ************************ */
    if (flag == 's') {
        ...
        /* EDIT FOR EACH MODEL */        
        /* Compute dynamics: */
<* CAssign[{out[3], out[4], out[5]}, {x5dotdot, x6dotdot, x7dotdot}] *>
        /* End dynamics. */
        
        /* The last half of the input is the first half of the output */
        for (i=0 ; i<dim/2 ; i++) {
            if (isnan(out[i+dim/2]) || isinf(out[i+dim/2])) {
        ...
        /* EDIT FOR EACH MODEL */
        /* Compute impact: */
<* CAssign[{out[3], out[4], out[5]}, {x5dotimpact, x6dotimpact, x7dotimpact}] *>
        /* End impact. */
