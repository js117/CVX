...
/* Parse inputs */
/* ************ */
    if (nrhs == 0 || &nrhs == NULL) dieHorribly(0);
...
/* ************************ */
    if (flag == 's') {
        ...
        tmp[10] = 2.*l*q[3]*(-3.*M - 4.*Mp + 2.*M*cos(2.*q[2]))*(q[4]*((5.*M + 4.*Mp)*sin(2.*q[1]) + M*(sin(2.*(q[1] + q[2])) - 4.*sin(2.*q[1] + q[2]))) + M*q[5]*(-2.*sin(q[2]) + sin(2.*(q[1] + q[2])) - 2.*sin(2.*q[1] + q[2])));
        ...
        tmp[23] = sin(q[2]);
        tmp[24] = 16.*l*w*q[3]*(q[4]*((5.*M + 4.*Mp)*sin(2.*q[1]) + M*(sin(2.*(q[1] + q[2])) - 4.*sin(2.*q[1] + q[2]))) + M*q[5]*(-2.*sin(q[2]) + sin(2.*(q[1] + q[2])) - 2.*sin(2.*q[1] + q[2])))*((M + Mp)*sin(q[1]) - M*sin(q[1] + 2.*q[2]));
        tmp[25] = q[4]*q[4];
        ...
        tmp[47] = sin(q[2]);
        tmp[48] = 16.*l*w*q[3]*((M + Mp + (M + 2.*Mp)*cos(q[2]) - M*cos(2.*q[2]))*sin(q[1]) + cos(q[1])*(5.*M + 4.*Mp - 2.*M*cos(q[2]))*sin(q[2]))*(q[4]*((5.*M + 4.*Mp)*sin(2.*q[1]) + M*(sin(2.*(q[1] + q[2])) - 4.*sin(2.*q[1] + q[2]))) + M*q[5]*(-2.*sin(q[2]) + sin(2.*(q[1] + q[2])) - 2.*sin(2.*q[1] + q[2])));
        ...
        qnew[0] = (-(q[3]*(tmp[5] + 2.*(tmp[6] + tmp[7] + tmp[8]))) + tmp[9])/(tmp[1] + 2.*(tmp[2] + tmp[3] + tmp[4]));
        tmp[1] = 1/l;
        ...
        qnew[1] = (tmp[1]*(-(l*(M*q[5]*(tmp[6] + tmp[7]) + q[4]*(tmp[8] + tmp[9] + tmp[10]*(tmp[11] + tmp[12]) + tmp[13]*tmp[14]))) + q[3]*tmp[15]*((tmp[16] + tmp[17] + tmp[18])*tmp[19] + tmp[20] + tmp[21] + tmp[22])))/(tmp[2] + 2.*(tmp[3] + tmp[4] + tmp[5]));
        tmp[1] = 1/l;
        tmp[2] = cos(2.*q[1])*(cos(2.*q[2])*(l*l*(32.*M*Mp + 13.*(M*M) + 16.*(Mp*Mp)) + 4.*Mp*(2.*M + Mp)*(w*w)) + M*(-2.*M*(l*l) - (5.*M + 4.*Mp)*cos(4.*q[2])*(l*l) + 4.*(M + 2.*Mp)*(w*w)));
        tmp[3] = (3.*M + 2.*Mp)*(3.*M + 4.*Mp)*(l*l) + (7.*M*Mp + 2.*(M*M) + 2.*(Mp*Mp))*(w*w);
        tmp[4] = 2.*M*cos(2.*q[2])*(-((3.*M + 2.*Mp)*(l*l)) + Mp*(w*w));
        ...
        qnew[2] = (tmp[1]*(-(l*(M*q[5]*(tmp[6] + tmp[7]) + 4.*q[4]*(2.*(tmp[8] + tmp[9])*tmp[10] + tmp[11]))) + q[3]*tmp[12]*(tmp[13] + tmp[14] + tmp[15] + tmp[16] + tmp[17] + tmp[18])))/(tmp[2] + 2.*(tmp[3] + tmp[4] + tmp[5]));
        /* End impact. */
        plhs[0] = mxCreateDoubleMatrix(dim, 1, mxREAL);
        if (plhs[0] == NULL) dieHorribly(-1);
    ...
    /* Compute guard function: */
        guard = -(w*sin(q[0])) + 2.*l*cos(q[0])*sin(q[1] + 0.5*q[2])*sin(0.5*q[2]);
    /* End guard function. */
